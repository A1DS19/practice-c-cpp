...
inline void ImGui::FileBrowser::Display() {
    ...
    if (SmallButton("*")) {
        ...
        for (auto& name : selectedFilenames_) {
            const auto it =
                std::find_if(fileRecords_.begin(), fileRecords_.end(),
...
inline bool
ImGui::FileBrowser::SetCurrentDirectoryInternal(const std::filesystem::path& dir,
                                                const std::filesystem::path& preferredFallback) {
    try {
...
inline void ImGui::FileBrowser::AssignToArrayStyleString(std::vector<char>& arr,
